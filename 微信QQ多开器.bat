...
:OPEN_WECHAT
    ...
    if !num_wechat! gtr 0 (
        ...
        :: 调用函数启动指定数量的微信
        call :LAUNCH_APP_MULTI_USER "!wechat_path!" !num_wechat! "微信"
    ) else (
        echo 已取消打开微信或数量为0。
    )
    pause
...
:OPEN_BOTH
    ...
    if !num_wechat_both! gtr 0 (
        ...
        :: 调用函数启动指定数量的微信
        call :LAUNCH_APP_MULTI_USER "!wechat_path!" !num_wechat_both! "微信"
    ) else (
        echo 微信数量为0，跳过打开微信。
    )
    pause
...
:LAUNCH_APP_MULTI_USER
    ...
    for /L %%i in (1,1,%~2) do (
        echo 正在启动第 %%i 个 %~3...
        :: 使用不同的用户数据目录启动微信
        start "" "%~1" /portable:"C:\PortableApps\WeChat%%i"
        :: 稍微延迟一下，避免同时启动过多导致系统卡顿
        ::timeout /t 1 /nobreak >nul
    )
    echo %~2 个 %~3 已全部尝试启动。
